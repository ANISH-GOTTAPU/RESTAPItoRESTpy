...
class QuickTest(object):
    ...
    def getAllQuickTestHandles(self):
        ...
        Returns:
            [   <ixnetwork_restpy.testplatform.sessions.ixnetwork.quicktest.openflowlayer2learningrate_3db88746f7f375303f2eda376386a9a8.OpenFlowLayer2LearningRate object at 0x03AAE2B0>]
        """
        quickTestObjects = []
        for testId in self.ixNetwork.QuickTest.TestIds:
            qtType = testId.split("/")[-2]
            qtType = qtType[0].upper() + qtType[1:]
            qtobj = eval("self.ixNetwork.QuickTest." + qtType + ".find()")
            quickTestObjects.append(qtobj)
    ...
    def getQuickTestNameByHandle(self, quickTestHandle):
        ...
        Parameter :
            quickTestHandle = <ixnetwork_restpy.testplatform.sessions.ixnetwork.quicktest.rfc2544throughput_5a77c9a28f5fa2bb9ce9f4280eb5122f.Rfc2544throughput object at 0x03800D00>
        """
    ...
    def applyQuickTest(self, qtHandle):
        ...
        try :
            ...
            raise Exception("Operation apply quicktest failed")

    def getQuickTestCurrentAction(self, quickTestHandle):
        """
        Description :
            Returns current action like 'InitializingTest', 'ApplyFlowGroups', 'SetupStatisticsCollection', etc.
        Parameter :
            quickTestHandle = <ixnetwork_restpy.testplatform.sessions.ixnetwork.quicktest.rfc2544throughput_5a77c9a28f5fa2bb9ce9f4280eb5122f.Rfc2544throughput object at 0x03800D00>
        """
        timer = 10
    ...
    def getQuickTestResult(self, quickTestHandle, attribute):
        ...
        result = eval ('quickTestHandle.Results.' + attribute)
        return result
    ...
    def getQuickTestPdf(self, quickTestHandle, copyToLocalPath, where='remoteLinux', renameDestinationFile=None, includeTimestamp=False):
        ...
        try :
            ...
            raise Exception("Generate quicktest report file failed")

        if where == 'localWindows':
            self.fileMgmtObj.copyFileWindowsToLocalWindows(reportfile, copyToLocalPath, renameDestinationFile, includeTimestamp)
        if where == 'remoteWindows':
            self.fileMgmtObj.copyFileWindowsToRemoteWindows(reportfile, copyToLocalPath, renameDestinationFile, includeTimestamp)
        if where == 'remoteLinux':
            self.fileMgmtObj.copyFileWindowsToLocalLinux(reportfile, copyToLocalPath, renameDestinationFile, includeTimestamp)
    ...
    def runQuickTest(self, quickTestName, timeout=90):
        ...
        try :
            ...
            raise Exception("Run quicktest operation failed")
    ...
    def deleteQuickTest(self, quickTestName):
        ...
        try:
            ...
            self.ixnObj.logInfo("Delete quicktest by quicktest name failed")
    ...
    def configQuickTest(self, quickTestName, numOfTrials=1):
        ...
        try :
            ...
            raise Exception ("Unable to configure quick test")