...
class Connect:
    ...
    def __init__(self, apiServerIp=None, serverIpPort=None, serverOs='windows', linuxChassisIp=None,
        ...
        #         raise IxNetRestApiException('If using windowsConnectionMgr, you must state httpsSecured=True|False
        #         and a serverIpPort to use.')
        #
        ...
        # # This is the only api server that requires user to state httpsSecured=True|False because 8.40 users could be
        # using http.
        ...
        #
        #     if sessionId:
        #         url = '{0}://{1}:{2}/api/v1/sessions/{3}'.format(self.httpScheme, apiServerIp, self.apiServerPort,
        #         str(sessionId))
        #         try:
        ...
        #                 url = '{0}://{1}:{2}/api/v1/sessions/{3}'.format(self.httpScheme, apiServerIp,
        ...
        #         if self.apiKey == None:
        #             response = self.post('https://{}:{}/api/v1/auth/session'.format(self.linuxApiServerIp,
        #             self.apiServerPort),
        #                                  data={"username": "admin", "password": "admin"},
        #                                  headers={'content-type': 'application/json'})
        #
        #         self.jsonHeader = {'content-type': 'application/json', 'x-api-key': self.apiKey}
        #         self.sessionUrl = 'https://{0}:{1}/ixnetworkweb/api/v1/sessions'.format(self.linuxApiServerIp,
        #         self.apiServerPort)
        #         return
        #
        ...
        #         self.jsonHeader = {'content-type': 'application/json', 'x-api-key': self.apiKey}
        #
        #         response = self.get('https://{0}:{1}/api/v1/sessions/{2}'.format(self.linuxApiServerIp,
        #         self.apiServerPort, str(sessionId)))
        #
        ...
        #             self.httpHeader = self.sessionUrl.split('/ixnetworkweb')[0]
        #
        #     # Create new session: connectToLinuxApiServer API knows whether to create a new session or connect to an
        #     existing session by looking at the self.apiKey.
        #     if apiKey == None and sessionId == None:
        #         self.connectToLinuxApiServer(self.linuxApiServerIp, self.apiServerPort, username=username,
        #         password=password, verifySslCert=verifySslCert, timeout=self.linuxApiServerTimeout)
        #
        ...
        # if deleteSessionAfterTest = True.
    ...
    def get(self, restApi, data={}, stream=False, silentMode=False, ignoreError=False, maxRetries=5):
        ...
        while True:
            ...
            try:
                ...
                if not str(response.status_code).startswith('2'):
                    if not ignoreError:
                        if 'message' in response.json() and response.json()['messsage'] is not None:
    ...
    def createWindowsSession(self, ixNetRestServerIp, ixNetRestServerPort='11009'):
        ...
        #         response = self._session.request('POST', url, data={}, verify=self.verifySslCert,
        #         allow_redirects=True)
        #
        #     except requests.exceptions.RequestException as errMsg:
    ...
    def waitForComplete(self, response='', url='', silentMode=False, ignoreException=False, httpAction='get',
        ...
        #     self.logInfo("\tState: %s " % response.State, timestamp=False)
        #
        # if not response.json():
        #     raise IxNetRestApiException('waitForComplete: response is empty.')
        #
        # if response.json() == '' or response.State == 'SUCCESS':
        #     return response
        #
        # if 'errors' in response.json():
        #     raise IxNetRestApiException(response.Errors)
        #
    ...
    def showLicenseDetails(self):
        ...
        self.logInfo('\nVerifying sessionId license server: %s' % self.httpHeader + self.ixNetwork, timestamp=False)
    ...
    def getAllOpenSessionIds(self):
        ...
        for session in response.json():
            ...
            sessionIdDict[id] = {'id': id, 'sessionIdUrl': href, 'username': username, 'state': state,
                                 'createdOn': createOn}
    ...
    def refreshHardware(self, chassisObj):
        ...
        """
        # response = self.post(self.sessionUrl+'/availableHardware/chassis/operations/refreshinfo',
    ...
    def getMultivalueValues(self, multivalueObj, silentMode=False):
        ...
        #         'arg3': count
        #         }
        # response = self.ixnObj.post(self.ixnObj.sessionUrl+'/multivalue/operations/getValues', data=data,
        # silentMode=silentMode)
        # self.ixnObj.waitForComplete(response, self.ixnObj.sessionUrl+'/operations/multivalue/getValues'
        # +response.json()['id'], silentMode=silentMode)
